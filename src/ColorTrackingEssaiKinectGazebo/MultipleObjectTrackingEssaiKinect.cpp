...
{
	double TempDepthObject = 0;
	double MaxDepthObject;
	double DifferenceDepth;
	double MinDepthObject;
	double TempMaxX;
	double TempMaxY;
	float width =0;
	float height=0;
	...
	{
        TempDepthObject = cloud(SceneObjects.at(i).getXPos(),SceneObjects.at(i).getYPos()).z;
		MinDepthObject = cloud(SceneObjects.at(i).getXPos(),SceneObjects.at(i).getYPos()).z;

		vector<Point> PointDepart;
		/*PointDepart.x =  SceneObjects.at(i).getXPos();
		//PointDepart.y =  SceneObjects.at(i).getYPos();

		/*for (int j = 0; j<contours[0].size()-1; j++)
		{
			
			//cout << "j " << contours[0][1].x << endl;
			//cout << "j " << contours[0][contours[0].size()-1].x << endl;
			if (contours[0][j].x < 640 && contours[0][j].y < 480)
			{
				TempDepthObject = cloud(contours[0][j].x,contours[0][j].y).z;

				if (MaxDepthObject < TempDepthObject) { MaxDepthObject = TempDepthObject;}
				if (MinDepthObject > TempDepthObject) { MinDepthObject = TempDepthObject;}
			}
		}
		DifferenceDepth = MaxDepthObject - MinDepthObject;
		cout << "DifferenceDepth " << DifferenceDepth << endl;
		cout << "MinDepth " << MinDepthObject<< endl;
		cout << "MaxDepth " << MaxDepthObject<< endl;*/
		...
		MesObjects = j.advertise<mypkg::Object3D>("/Objets", 10);
		mypkg::Object3D MessageObjet;
		MessageObjet.x = Z;
		MessageObjet.y = -X;
		MessageObjet.z = -Y;
		MessageObjet.type = SceneObjects.at(i).getType();
		MesObjects.publish(MessageObjet);
