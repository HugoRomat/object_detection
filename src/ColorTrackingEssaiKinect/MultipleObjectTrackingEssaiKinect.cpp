...
{//This function gets called whenever a
	...
	Then, for the bad points (where no depth information is available), use NaNs :
...
/*void callbackCloud(const sensor_msgs::PointCloud2ConstPtr& cloud)
{
	PointCloud = pcl_conversions::toPCL(cloud);
...
void CreatePublisherObject()
{
	...
	for(int i = 0; i<SceneObjects.size(); i++)
	{
		...
		//depth0 = cv_Depth->image.at<short int>(cv::Point(SceneObjects.at(i).getXPos()-320,SceneObjects.at(i).getYPos()));
		//depth0 = depth0/1000;
		//depthObject = depthObject;
		//Resolution Kinect 640 x 480

		//double VecteurX = (double)depthObject/1000;
		//double VecteurY = -((double)SceneObjects.at(i).getXPos()-320);
		//double VecteurZ = -((double)SceneObjects.at(i).getYPos()-240);
		

		/*double fx_d = 1.0 / 5.9421434211923247e02;
		double fy_d = 1.0 / 5.9104053696870778e02;
		double cx_d = 3.3930780975300314e02;
		double cy_d = 2.4273913761751615e02;
	
		//double depth = rawDepthToMeters(depthObject);
		VecteurY = (float)((VecteurY - cx_d) * VecteurX  * fx_d);
		VecteurZ = (float)((VecteurZ - cy_d) * VecteurX  * fy_d);
		VecteurX = (float)(VecteurX);
		VecteurY = VecteurY/1000;
		VecteurX = VecteurX/1000;
		VecteurZ = VecteurZ/1000;*/
		//0,545
		
		//CA MARCHE
		/*double zw = depthObject * 525 / (sqrt(pow(VecteurX,2) + pow(VecteurY,2) + pow(525,2)));
		VecteurY = zw * VecteurY / 525;
		VecteurY = VecteurY/1000;

		VecteurZ = zw * VecteurZ / 525;
		VecteurZ = VecteurZ/1000;

		zw = zw / 1000;*/
		

		//VecteurY = VecteurY / (0.555 - (((depthObject-997)*0.01)/17));
		//VecteurY = VecteurY / 0.53 ;//- (((depthObject-997)*0.01)/17));
		//VecteurY=sqrt(pow(depthObject-0.05,2) - pow(depth0, 2));
	    //VecteurY = ((VecteurY*depthObject*0.9)/(double)focalLengthY)/1000;
		//VecteurZ = VecteurZ/1000;
		//VecteurY=VecteurY/1000;
		//VecteurZ=(VecteurZ/37.795275)/100;		
		double X = cloud(SceneObjects.at(i).getXPos(),SceneObjects.at(i).getYPos()).x;
		double Y = cloud(SceneObjects.at(i).getXPos(),SceneObjects.at(i).getYPos()).y;
		double Z = cloud(SceneObjects.at(i).getXPos(),SceneObjects.at(i).getYPos()).z;
		

		cout << "=============================" << endl;
		//cout << "Depth0 " << zw << endl;
		//cout << "DepthObject " << depthObject << endl;
	    cout << "VecteurDebutX " << Z << endl;
		cout << "VecteurDebutY " << -X << endl;
		cout << "VecteurDebutZ " << -Y << endl;

	 	tf::Vector3 pointKinect(Z,-X,-Y);
		...
		cout << "=============================" << endl;
	    cout << "pointOdom.getX() " << pointOdom.getX() << endl;
		cout << "pointOdom.getY() " << pointOdom.getY() << endl;
		cout << "pointOdom.getZ() " << pointOdom.getZ()  << endl;


        ss << pointOdom.getX() << "/" << pointOdom.getY() << "/" << pointOdom.getZ() << "/" << SceneObjects.at(i).getType() << "/" << width << "/" << height << "/" << i;
...
int main(int argc, char* argv[])
{
	...
	//}
	//image_transport::ImageTransport Cloud(n);
	//image_transport::Subscriber subCloud = Cloud.subscribe("/camera/depth/points",1,callbackCloud);
	//ros::Subscriber subCloud = n.subscribe("/camera/depth_registered/points",100,callbackCloud);
